/**
 * polygon-tool.js - Regular polygon creation tool using Paper.js
 * Fresh implementation - click+drag from center to define radius
 */
...
(function() {
    ...
    const tool = {
        ...
        onMouseMove(e, svgCanvas) {
            ...
            if (radius > 5) {
                state.drawingPath = new paper.Path.RegularPolygon({
                    center: [state.dragStart.x, state.dragStart.y],
                    sides: state.polygonSides || 6,
                    radius: radius,
                    strokeColor: '#00d4aa',
                    fillColor: null,
                    strokeWidth: 2,
        ...
        onMouseUp(e, svgCanvas) {
            ...
            const pp = new paper.Path.RegularPolygon({
                center: [state.dragStart.x, state.dragStart.y],
                sides: state.polygonSides || 6,
                radius: radius
            });
    ...
    window.VectorEditor.tools.polygon = tool;
